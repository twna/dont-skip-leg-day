...
const PunishmentMode = ({ onComplete, onCheatingDetected }) => {
  ...
  const [brightnessDiff, setBrightnessDiff] = useState(0);
  ...
  const analyzeMotion = () => {
    ...
    try {
      ...
      const SQUAT_THRESHOLD = 0.2; // 20% brightness drop to count as a squat - more sensitive
      const RETURN_THRESHOLD = 0.1; // 10% threshold for returning to up position
      const timeSinceLastFrame = now - lastFrameTimeRef.current;
      
      // Update debug values
      setCurrentBrightness(Math.round(avgBrightness));
      setBrightnessDiff(Math.round(brightnessDrop * 100));
      
      console.log(`Brightness: ${Math.round(avgBrightness)}, Baseline: ${Math.round(baselineBrightnessRef.current)}, Drop: ${Math.round(brightnessDrop * 100)}%, State: ${squatStateRef.current}`);
      
      // Simple state machine for squat detection
      if (brightnessDrop > SQUAT_THRESHOLD && squatStateRef.current === 'up' && timeSinceLastFrame > 500) {
      ...
      else if (brightnessDrop < RETURN_THRESHOLD && squatStateRef.current === 'down' && timeSinceLastFrame > 500) {
        // Transition back to up state - complete squat
        squatStateRef.current = 'up';
        lastFrameTimeRef.current = now;
        console.log('SQUAT UP DETECTED - COUNTING REP');
  ...
  const calculateBrightness = (imageData) => {
    ...
    // Use better sampling - consider more pixels for accuracy
    // Process every 10th pixel (instead of every 20th)
    ...
    // Calculate average brightness - scale by 10 due to sampling
      ...
      <div className="webcam-container">
        ...
        {isTracking && (
          <div style={{
            ...
            <div>Brightness: {currentBrightness}</div>
            <div>Brightness Drop: {brightnessDiff}%</div>
            <div>State: {squatStateRef.current}</div>
          </div>
        )}
